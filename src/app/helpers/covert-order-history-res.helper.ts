...
export const covertOrderHistoryRes = (res: OrderHistoryRes): FeedState => {
  return {
    ...
    ordersInProgress: res.orders.filter((item) => item.status === OrderStatusState.PENDING).map((item) => item.number),
    ordersInDone: res.orders.filter((item) => item.status === OrderStatusState.DONE).map((item) => item.number),
    total: res.total,
